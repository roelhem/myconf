...
{ doomemacs-config = import ./doomemacs-config.nix; }
#+end_src
...
(defun rh/random-uuid ()
  "Returns a random uuid in uppercase.

This is using the 'uuidgen' cli tool under the hood."
  (substring (shell-command-to-string rh/uuidgen-bin-path) 0 -1))

(defun rh/random-uuid-lowercase ()
  "Returns a random uuid in lowercase.

This is using the 'uuidgen' cli tool under the hood."
  (downcase (rh/random-uuid)))

(defun rh/insert-random-uuid ()
  "Insert an uppercase UUID."
  (interactive)
  (insert (rh/random-uuid)))

(defun rh/insert-random-uuid-lowercase ()
  "Insert a lowercase UUID."
  (interactive)
  (insert (rh/random-uuid-lowercase)))
#+end_src
...
#+begin_src snippet :tangle "snippets/haskell-mode/Text"
...
#+begin_src snippet :tangle "snippets/haskell-mode/TextLazy"
...
#+begin_src nix :tangle "doomemacs-config.nix"
{
  files = [./packages.el ./config.el ./shared-config.el];
}
#+end_src
